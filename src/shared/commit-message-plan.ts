...
} from './commit-message-plan-singleton-options'
...
export function planCommitMessageGeneration(
  ...
  if (!isTuiAgentAllowedForBuildProfile(input.agentId, buildProfile)) {
    return {
      ok: false,
      error: `Agent "${input.agentId}" is not allowed by the corporate build profile.`
    }
  }
